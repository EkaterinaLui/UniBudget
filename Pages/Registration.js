...
function Registration({ navigation, db }) {
  ...
  const handleRegister = async () => {
    ...
    if (!userName || !email || !password || !repetPassword) {
    ...
    try {
      ...
      await setDoc(doc(db, "users", user.uid), {
        ...
        name: userName.trim(),
        uniqueCode,
        createdAt: serverTimestamp(), // ✅ זמן שרת במקום new Date()
      });

      Alert.alert("נרשמת בהצלחה!", "", [
        {
          text: "OK",
          onPress: () => navigation.replace("Home"), // ✅ לוקח ישירות למסך הבית/ראשי
        },
      ]);
    } catch (firebaseError) {
      ...
      >
        <View style={styles.container}>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.7}
          >
            <Text style={styles.linkText}>כבר יש לך חשבון? להתחבר</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
    </KeyboardAvoidingView>
